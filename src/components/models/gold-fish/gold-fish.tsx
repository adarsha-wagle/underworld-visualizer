...
export type TGoldfishBehavior = "swim" | "stopAndMove" | "zigzag";

export interface IGoldfish extends IDynamicModel {
...
const updateSwim = (goldfish: IGoldfish, deltaTime: number): void => {
...
const updateStopAndMove = (goldfish: IGoldfish, deltaTime: number): void => {
...
const updateZigzag = (goldfish: IGoldfish, deltaTime: number): void => {
  const amplitude = 4;
  const frequency = 0.1;

  const sideVector = new THREE.Vector3().copy(goldfish.direction);
  sideVector
    .applyAxisAngle(new THREE.Vector3(0, 1, 0), Math.PI / 2)
    .normalize();

  const time = goldfish.behaviorTimer;
  const offsetAmount = Math.sin(time * frequency * Math.PI * 2) * amplitude;

  const targetOffset = sideVector.multiplyScalar(offsetAmount);
  goldfish.position.add(targetOffset.clone().multiplyScalar(deltaTime));
