...
const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("access_key", "e682ed52-6997-4d0f-a731-0a1b1eacc415");
  ...
  useEffect(() => {
    ...
    if (alert?.type === "success") {
      t = setTimeout(() => setAlert(null), 3000);
    }
...
function StatusAlert({ type, title, description }: Props) {
  ...
  return (
    ...
    >
      {isSuccess ? (
        <CheckCircle2 className="w-5 h-5 fill-cyan-300 mt-0.5" />
      ) : (
        <AlertTriangle className="w-5 h-5 fill-red-400 mt-0.5" />
      )}

      <AlertTitle
        className={`font-semibold tracking-wider ${
          isSuccess ? "text-cyan-50" : "text-red-200"
        }`}
      >
        {title}
      </AlertTitle>
